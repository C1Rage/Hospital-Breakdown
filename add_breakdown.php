...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $query = "INSERT INTO eapip_breakdown (
                    date, or_number, particulars, total_amount, breakdown,
                    clearveu, fas, fresenius, infimax, ivaxx, 
                    macrik, mahintana, red_cross, russan, sannovex, 
                    twincirca, zion, otr_paybls_pf, hospital_fees
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param(
            ...
            $consignors[13], $consignors[14], $consignors[15], $consignors[16], $consignors[17], 
            $consignors[18], $consignors[19], $consignors[20], $consignors[21], $consignors[22], 
            $consignors[23], $consignors[24], $consignors[28], $hospital_fees
        );
        ...
        // Get the last inserted ID from eapip_breakdown
        $eapip_bd_id = $conn->insert_id;

        echo "Breakdown successfully added. ID: $eapip_bd_id<br>";
        $stmt->close();

        // Insert data into consignor table with eapip_bd_id
        $consignor_query = "INSERT INTO consignor (consignor_id, or_number, cr, date, eapip_bd_id) VALUES (?, ?, ?, ?, ?)";
        ...
        foreach ($consignors as $consignor_id => $amount) {
            if ($amount > 0) {
                ...
                $consignor_stmt->bind_param("isssi", $consignor_id_int, $orNumberStr, $amount_float, $date, $eapip_bd_id);
